...
class QuestionAnswer:#{{{
    ...
    def store(self, categoryid):
        questionEntry = {'category_id':categoryid,
                    'value':self.value,
                    'question':self.question.encode('UTF-8'),
                    'answer':self.answer.encode('UTF-8'), }
        #try:
        db.questions.insert(questionEntry)
        #except:
...
class Category:#{{{
    ...
    def store(self):
        catEntry = {'name':self.name,'game_id':self.game_id}
        theid = db.categories.insert(catEntry)
        self.dbid = theid
